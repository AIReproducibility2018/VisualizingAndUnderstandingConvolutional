...
def load_validation_data(wnids, labels):
    ...
    for i in range(NR_VALIDATION_IMAGES):
        image = cv2.imread(VALIDATION_IMAGE_FOLDER + files[i])
        X_val.append(image)
...
def train_model():
    ...
    val_acc = 0.0
    val_batches = 0
    for inputs, targets in create_validation_batches(X_val, y_val, BATCH_SIZE):
    ...
    previous_val_acc = 0
    print("Initial validation accuracy:\t\t{:.4f} %".format(validation_acc))
    ...
    for epoch in range(9, EPOCHS):
        ...
        for inputs, targets in create_training_batches(X_train, y_train, BATCH_SIZE, mean_matrix):
            results = model.train_on_batch(inputs, targets)
            loss = results[0]
            acc = results[1]
            train_acc += acc
            train_loss += loss
        ...
        training_acc = train_acc / train_batches
        ...
        #if abs(validation_acc - previous_val_acc) < 0.00001:
        #    previous_val_acc = validation_acc
        #    learning_rate /= 10
        #    model.optimizer.lr = learning_rate

        # Save model
        model.save("models/epoch_" + str(epoch) + ".h5")
